---
name: pink-glass
description: Install, enable, disable and customize the Codex Custom Themes for Windows visual mod for Codex on Windows, including Sakura, Lavender, Moonlight, Peach and Mint themes. Use when the user asks to manage this mod or its appearance.
---

# Codex Custom Themes for Windows

Formerly Pink Glass. The compatible plugin identifier remains `pink-glass`. All saved profiles and launcher paths retain their legacy identifiers.
...
- `-Action Install`: verify package hashes, copy runtime to LOCALAPPDATA/PinkGlassPlugin/version-packageHash, and create/update the desktop `Codex Custom Themes for Windows` shortcut without starting or closing Codex. It prints verified physical paths that also work outside the packaged app.
