 ...
 try{
  for(const native of ['light','dark'])for(const theme of ['sakura','lavender','moonlight','peach','mint']){
   ...
   const sample=()=>page.evaluate(()=>{
    ...
    return {header:pair('.share span','.share'),title:pair('header .title','header .title'),body:pair('.body-copy','.thread-scroll-container'),secondary:pair('article .secondary','.thread-scroll-container'),link:pair('article a','.thread-scroll-container'),composer:{fg:rgba(getComputedStyle(q('textarea'),'::placeholder').color),bg:rgba(s(q('.composer-surface-chrome')).backgroundColor)},sidebar:{fg:rgba(s(q('.nav:not([aria-current])')).color),bg:rgba(s(q('aside')).getPropertyValue('--pink-wallpaper-scrim')||s(q('aside')).backgroundColor)},sidebarSecondary:{fg:rgba(s(q('.section')).color),bg:rgba(s(q('aside')).getPropertyValue('--pink-wallpaper-scrim')||s(q('aside')).backgroundColor)},headerBackground:s(q('header')).backgroundColor,readerBackground:s(q('.thread-scroll-container')).backgroundColor};
   });
   const before=await sample();assert.equal(before.headerBackground,'rgba(0, 0, 0, 0)');
   const values=[];
   for(const name of theme==='moonlight'?['header','title','composer']:['header','title','body','secondary','link','composer','sidebar','sidebarSecondary']){
   ...
   await page.locator('.share').hover();const hover=(await sample()).header;assert.ok(contrast(hover.fg,composite(hover.bg,[0,0,0]))>=4.5);
   ...
   if(theme==='moonlight')assert.equal(before.readerBackground,'rgba(0, 0, 0, 0)','Keep the existing dark reading area');
   ...
   const reduced=await sample();for(const name of ['header','body','composer','sidebar'])assert.equal(reduced[name].bg[3],255,`${theme}/${name}/reduced transparency`);
   await cdp.detach();
