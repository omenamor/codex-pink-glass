...
@layer native { .native-composer:not([data-composer-utility-bar-variant="home"]), [data-composer-utility-bar-variant="home"] .body { border:0!important; } }
...
test('full inset contour survives native layered border reset in all composer layouts',async()=>{
 ...
 try{for(const theme of ['sakura','lavender','moonlight','peach','mint']){
  ...
  for(const id of ['regular','home-descendant','home-self']){
   ...
   assert.equal(value.border,'0px','A native reset or home styling prevents a second border');
  ...
  const regularShadow=await page.locator('#regular').evaluate(e=>getComputedStyle(e).boxShadow);
  for(const id of ['home-descendant','home-self'])assert.equal(await page.locator('#'+id).evaluate(e=>getComputedStyle(e).boxShadow),regularShadow,`${theme}: home contour and shadow match regular composer`);
  await page.getByRole('textbox',{name:'Поле на главной',exact:true}).focus();assert.equal(await page.locator('#home-descendant').evaluate(e=>getComputedStyle(e).boxShadow),regularShadow,'Focusing home input must not add a second accented ring');
